...
class Tools():
	...
	def customLCN(self, services, section_identifier, current_bouquet_key, provider_config=None):
		...
		for type in ["video", "radio"]:
			...
			# Determine sd/hd prefix: use provider_config flag if available, else fall back to key prefix
			if provider_config is not None:
				sdorhd = "sd" if provider_config.isSDStream() else "hd"
			else:
				sdorhd = "sd" if current_bouquet_key.startswith('sd') else "hd"
			xmlout = open(user_custom_dir + "/EXAMPLE_" + sdorhd + "_" + section_identifier + "_Custom" + ("radio" if type == "radio" else "") + "LCN.xml", "w")
			xmlout.write(''.join(xml_out_list))
			xmlout.close()
			del xml_out_list

			# Read CustomLCN file
			customfile = self.getCustomPath(sdorhd + "_" + section_identifier + "_Custom" + ("radio" if type == "radio" else "") + "LCN.xml")
